...
describe('MessageService', () => {
    ...
    before(async () => {
        ...
        user1 = await User.create({ name: 'User1', email: 'user1@test.com', password: 'hashedPassword1' });
        user2 = await User.create({ name: 'User2', email: 'user2@test.com', password: 'hashedPassword2' });
        user3 = await User.create({ name: 'User3', email: 'user3@test.com', password: 'hashedPassword3' });
    ...
    describe('createMessage', () => {
        it('should create a new message', async () => {
            const messageData = { user_id: user1._id, recipientId: user2._id, content: 'Hey!' };
    ...
    describe('getPeopleChat', () => {
        it('should return the chat between two specific users', async () => {
            ...
            expect(result[0]).to.have.property('user_id').that.satisfies(val => val.equals(user1._id) || val.equals(user2._id));
            expect(result[0]).to.have.property('recipientId').that.satisfies(val => val.equals(user1._id) || val.equals(user2._id));
        });
    });
    ...
    describe('deleteMessage', () => {
        it('should delete a message by ID', async () => {
            const message = await Message.create({ user_id: user1._id, recipientId: user3._id, content: 'Hello again!' });
